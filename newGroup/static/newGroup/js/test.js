...
if(editState != "true"){
   ...
   if(items){
    ...
    for(var i=0; i<items.length; i++){
     ...
     oOption.text = items.text;
     oOption.value = items.value;
...
function mddisplayDate(){
  document.getElementById("demo").innerHTML=Date();
       alert( document.getElementById('tabProduct').rows.cells[0].firstChild.checked);
...
for(var i=1; i<table.rows.length;i++){
   tableData.push(GetRowData(tabProduct.rows));
}
